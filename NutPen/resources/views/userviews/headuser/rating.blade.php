            ...
            <div class="tm-bg-primary-dark tm-block tm-block-taller tm-block-scroll">
                ...
                @else
                    @if ($status ==4)       <!--//Értékelések listázása a tanórában szereplő diákoknak osztályra szűrve-->
                        ...
                        <button class="NewItemButton" onclick="location.href = '/fo/tanorak/ujertekeles/{{ $lessonID }}/osztaly/{{ $classID }}';" >Új értékelés</button>
                        @include('gradeinfo')
                        <table id='dtBasicExample' class="table table-bordered table-striped table-sm ">
                            <thead>
                                <tr>
                    ...
                    @else
                        @if ($status ==5)       <!--//osztály tanórához felvétel-->
                            ...
                            <form id="ujFelh" class="formCenterContent" action="/fo/tanorak/ertekelesekmentes" method="post">
                                @csrf
                        ...
                        @else
                                    ...
                                    <tbody id="myTable">
                                        @foreach ($ratings as $item)
                                            <tr>
                                                ...
                                                <td><div class="btnplacer"><button class="EditButton" onclick="location.href = '/fo/ertekelesmodositas/{{ $item->ID }}';" >Módosítás</button></div></td>
                                                <td><div class="btnplacer"><button class="RemoveButton" onclick="location.href = '/fo/ertekelestorles/{{ $item->ID }}';" >Törlés</button></div></td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                </table>
                            @else
                                @if($status==7)      <!--//Diák jegyének módosítása-->
                                    ...
                                    <form id="ujFelh" class="formCenterContent" action="/fo/ertekelesmodositas" method="post">
                                        @csrf
